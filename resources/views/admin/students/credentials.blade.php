...
@section('content')
    ...
    <div class="flex-1 p-8 overflow-auto max-w-lg">
        @if ($credential)
            <div class="bg-amber-50 border border-amber-200 rounded-2xl p-5 text-sm text-amber-900">
                ...
                <dl class="space-y-2">
                    ...
                    <div><dt class="text-xs font-bold text-amber-800/70">Login (NIS)</dt><dd class="font-mono font-bold">{{ $credential['login'] }}</dd></div>
        ...
        <a href="{{ route('admin.students.index') }}" class="inline-block mt-6 text-sm font-bold text-navy-800">← Kembali ke daftar siswa</a>
    </div>
@endsection
