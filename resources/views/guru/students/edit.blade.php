...
@section('content')
    <div class="h-[74px] bg-white border-b border-slate-200 flex items-center gap-3.5 px-8 shrink-0">
        ...
        <div class="min-w-0">
            <div class="text-lg font-extrabold text-navy-900 tracking-tight truncate">Ubah Siswa — {{ $student->name }}</div>
            <p class="text-[11px] font-semibold text-slate-400 mt-0.5">NIS {{ optional($student->studentProfile)->nis }}</p>
        </div>
    </div>
